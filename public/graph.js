...
function generate() {
  ...
  xhr.open("POST", window.location.origin + '/generate', true);
...
function downloadPNG() {
  // Find the architecture SVG inside the large display container
  var svgElement = document.querySelector("#lgContainer svg");

  if (!svgElement) {
    alert("No generated architecture graph to export!");
    return;
  }
  ...
  canvas.width = bbox.width * 2; // Increase resolution
  ...
  img.onload = function () {
    ...
    link.download = "architecture_graph.png";
    link.click();
  };

  // Ensure correct encoding of the SVG
